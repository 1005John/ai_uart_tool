...
> 版本：v1.1 | 日期：2026-05-30 | 对应需求：REQUIREMENTS.md v1.0
...
├──────────────────────────────────────────────┤
│               数据层                          │
│  SQLite (ai_uart.db) + JSON (test_sets/)      │
│  Excel 测试数据 (23+ xlsx)                    │
├──────────────────────────────────────────────┤
│               外部集成                        │
...
**核心数据流**：创建用例集 → 添加用例 → 方案勾选用例集 → 执行 → FAIL 自动生成缺陷 → 提交灵畿
...
| 标签 | 左侧 | 右侧 |
|------|------|------|
| 📦 用例集库 | Radio 列表 + 新建/删除 | HTML 表格 + 表单添加/序号删除 |
| 📋 方案管理 | Radio + 🔄刷新 + 用例集 Checkbox | HTML 表格（只读，含用例集来源列）+ 执行按钮 |
| 🐛 缺陷管理 | 多维筛选 | 缺陷列表 + 灵畿提交 |
