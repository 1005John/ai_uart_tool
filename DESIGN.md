...
> 版本：v1.2 | 日期：2026-05-31 | 对应需求：REQUIREMENTS.md v1.0
...
| | 右 | Dataframe 可编辑表格（点击行选中）+ 保存/删除/上移/下移按钮 |
| 📋 方案管理 | 左 | Radio + 轮次/间隔 + 用例集 Checkbox（勾选即保存） |
| | 中 | Dataframe 用例表格（勾选→执行） + 状态 |
| | 右 | Textbox 执行日志（>>>发送 / <<<接收） |
| 🐛 缺陷管理 | 左 | Dataframe 缺陷列表 + 灵畿提交 |
| | 右 | Markdown 缺陷详情 |
